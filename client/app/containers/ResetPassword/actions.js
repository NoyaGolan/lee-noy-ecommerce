...
export const resetPassword = token => {
  return async (dispatch, getState) => {
    try {
      const rules = {
        password: 'required|regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d.*\\d.*\\d.*\\d)(?=.*[!@%$#^&*-_*]).{8,}$/',
        confirmPassword: 'required|regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d.*\\d.*\\d.*\\d)(?=.*[!@%$#^&*-_*]).{8,}$/|same:password',
      };
      ...
      const { isValid, errors } = allFieldsValidation(user, rules, {
        ...
        'regex.password': 'Password must have at least one lowercase letter, one uppercase letter, four numbers, and one special character (!@%$#^&*-_*). minimum length of 8 characters',
        'min.confirmPassword':
